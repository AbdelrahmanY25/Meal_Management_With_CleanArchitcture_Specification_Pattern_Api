...
### Application (`MealMangament.Application`)
...
### Infrastructure (`MealMangament.Infrastructure`)
...
| `PUT` | `/api/meals/update/{mealId}` | Update an existing meal |
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/AbdelrahmanY25/Meal_Managment_With_MealMangament_Specification_Pattern_Api.git
   cd clean-architecture-meal-api
   ```

2. **Configure the connection string**

   Set the `DefaultConnection` in `MealMangament/appsettings.json` or using User Secrets:
   ```bash
...
3. **Apply migrations**
   ```bash
   dotnet ef database update --project MealMangament.Infrastructure --startup-project MealMangament
   ```

4. **Run the application**
   ```bash
   dotnet run --project MealMangament
   ```
...
```
├── MealMangament.Domain/              # Domain Layer
│   └── Entities/
│       ├── Meal.cs
│       ├── MealOptionGroup.cs
│       └── OptionGroupItems.cs
│
├── MealMangament.Application/         # Application Layer
│   ├── Common/
...
│
├── MealMangament.Infrastructure/      # Infrastructure Layer
│   ├── Persistence/
...
│   └── DependencyInjection.cs
│
└── MealMangament/                     # API Layer (Presentation)
